...
class FRCGM:
    ...
    def dostaff(self):
        ...
        while (task != 1):
            print('')
            print("The Fletcher-Reeves conjugate gradient method")
            print('')
    ...
    def resolve(self):
        ...
        #dfd = self.get_dfd(begin)
        self.s.vector = self.get_dfd(begin)
        self.previous_s.vector = self.s.vector.copy()
        dfd = self.s.vector
        print("Get lambda...")
        ...
        while self.halting_check() and k <= 60 and self.norm(self.s.vector) > 0.1:
            k += 1
            print(k)
            dfd = self.mul(self.s.vector, clambda)
            x_w = self.sum(x_w, dfd)
            self.g.vector = self.get_gradient(x_w)
            self.previous_s.vector = self.s.vector.copy()
            self.s.vector = self.get_dfd(begin)
            self.s.showvector()
            self.previous_s.showvector()
            self.g.showvector()
    ...
    def get_dfd(self, begin):
        if begin:
            dfd = self.g.vector.copy()
            dfd = self.mul(dfd, -1.0)
        else:
            dfd = self.g.vector.copy()
            dfd = self.mul(dfd, -1.0)

            prev_dfd = self.previous_s.vector.copy()

            part_up = self.g.hvm(self.g, 20)
            part_down = self.previous_g.hvm(self.previous_g, 20)

            try:
                part_opr = part_up / part_down
            except ZeroDivisionError:
                part_opr = part_up / float('Inf')

            print("Part opr", part_opr)

            prev_dfd = self.mul(prev_dfd, part_opr)
            dfd = self.sum(dfd, prev_dfd)
        return dfd
